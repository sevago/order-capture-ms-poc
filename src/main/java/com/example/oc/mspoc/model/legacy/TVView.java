...
public class TVView extends ProductView {
	...
	protected String pictureLink = "img/telustv.jpg";
	...
	public  void buildComponentMap (SearchAssignedProductHierarchy searchAssignedProductHierarchy) {
		...
		viewGroup[1].setGroupName("Channels");
	...
	private void buildComponentMapRec(SearchAssignedProductHierarchy searchAssignedProductHierarchy) {
		...
		if (searchAssignedProductHierarchy.getParentAssignedProduct() != null) {
			...
			if ("TVTP".equals(productType)) { //Packs
				vgComp = this.getComponentMap()[0];
			} 
			else if ("TVCG".equals(productType)) { //Channels - only children of TVCG
//				String name = searchAssignedProductHierarchy.getParentAssignedProduct().getAssignedProductHeader().getName().getLocalizedValue();
//				if ("A La Carte".equals(name)) { //A La Carte
